jQuery(function () {
    ...
    if (dig_reg_wp_elem.length) {
        ...
        if (!dig_reg_wp_elem.hasClass("dokan-vendor-register")) {
            ...
            if (dig_reg_wp_elem.find(".dig_fs_reg_form_end").length === 0) {
    ...
    jQuery("#fs_code_dig").closest("form").addClass("login");
    jQuery("#digits_fs_code").closest("form").addClass("site_front-ResetPassword");
        ...
        jQuery.ajax({
            ...
            success: function (res) {
                    ...
                    if (!res.data.code) {
                        ...
                        if (res.data.error_msg) {
                            ...
                            if (res.data.error_type) {
                                shodjigMessage(res.data.error_msg, res.data.error_type);
                            } else {
                                shodjigErrorMessage(res.data.error_msg);
                            }
                            return;
                        }

                        if (res.redirect) {
                            shodjigLoginSuccessMessage();
                ...
                if (res == "1") {
                    ...
                    shodjigNoticeMessage(dig_mdet.pleasesignupbeforelogginin);
                } else if (res == -9) {
                    shodjigErrorMessage(dig_mdet.invalidapicredentials);
                } else {
                    shodjigErrorMessage(dig_mdet.invalidlogindetails);
                }
...
// phone form submission handler
    ...
    var mailsecond = jQuery(".dig_fs_mailsecond");
    ...
    var register = jQuery(".dig_fs_reg_form").closest(".register");
    if (dig_sortorder.length) {
        ...
        if (dig_mdet.mobile_accept > 0) {
            register.find("#reg_email").parent().attr({
                ...
                'class': 'dig-custom-field site_front-FormRow--wide form-row form-row-wide'
            });
        } else {
            register.find("#reg_email").parent().attr({
                'id': 'dig_cs_email',
    ...
    var fsform = jQuery("#fs_dig_reg_form").closest("form");
    var fs_checkout = jQuery(".site_front-form-login");
    if (fsform.length) {
        fsform.find('input[type="password"]').closest(".site_front-FormRow").remove();
        fsform.find('input[name="login"]').remove();
        fsform.find(".site_front-LostPassword").remove();
        fsform.find('#rememberme').closest('label').remove();
        fsform.find("#username").attr('mob', 1);

        if (fs_checkout.length) {
            fs_checkout.find('input[type="password"]').closest('.site_front-form-row').remove();
            fs_checkout.find(".form-row-first").removeClass("form-row-first");
            fs_checkout.find(".lost_password").remove();
            fs_checkout.find('#rememberme').closest('label').remove();
            fs_checkout.find('[name="login"]').remove();
            fs_checkout.find("#username").attr('mob', 1);
        }
    }


    var uc = jQuery("#dig_fs_check_page");
    if (uc.length) {
        uc = uc.parent();
        ...
        if (createAccount.length) {
            ...
            jQuery(".fs_check_dig_custfields").appendTo(createAccount);
        }
    }
...
//// Ultimate user
    ...
    var um_forgot = jQuery(".site_front-ResetPassword");
    if (um_forgot.length) {
    ...
    jQuery('.digits_um_forgotpass #um-submit-btn').on('click', function () {
        ...
        var ccode_field = cuForm.find('.dig_fs_logincountrycode');
        var uname = uname_field.val();
        var ccode = ccode_field.val();
        if (isNumeric(uname)) {

            if (um_forgot_pass == 3) {
                var pass = jQuery("#dig_fs_password").val();
                var cpass = jQuery("#dig_fs_cpassword").val();
                if (pass != cpass) {
                    shodjigErrorMessage(dig_mdet.Passwordsdonotmatch);
                    return false;
                }
    ...
    var fs_login_form = jQuery(".site_front-form-login");
    if (fs_login_form.length) {
        if (dig_mdet.login_mobile_accept == 1 && dig_mdet.login_mail_accept == 0 && dig_mdet.login_uname_accept == 0) {
            fs_login_form.find('#username').data('type', 2);
        }
    }
    ...
    jQuery("#fs-pos-actions").find("#add_customer_to_register").on('click', function () {
        setTimeout(function () {
    ...
    function update_username_field($this) {
        ...
        usernameid.wrap('<div class="digcon"></div>').before('<div class="dig_fs_countrycodecontainer dig_fs_logincountrycodecontainer">' +
            '<input type="text" autocomplete="tel-country-code" name="' + dig_ccd_name + '" class="' + dig_ext + 'input-text countrycode dig_fs_logincountrycode" ' +
        ...
        usernameid.on("keyup change focusin", function (e) {
            ...
            var dclcc = jQuery(this).parent().find('.dig_fs_countrycodecontainer');
    ...
    jQuery(".site_front-EditAccountForm").find("#reg_email").addClass('no-overwrite');
    jQuery('input[id="reg_email"]').each(function (index) {
        ...
        reg_email.wrap('<div class="digcon"></div>')
            .before('<div class="dig_fs_countrycodecontainer dig_fs_registercountrycodecontainer"><input type="text" name="digfcountrycode" class="input-text countrycode dig_fs_registercountrycode" value="' + dig_mdet.uccode + '" maxlength="6" size="3" placeholder="' + dig_mdet.uccode + '" autocomplete="tel-country-code"/></div>');
        ...
        reg_email.on("keyup change focusin", function (e) {
            ...
            var dclcc = reg_input.find('.dig_fs_countrycodecontainer');
            var dcllInput = dclcc.find('input');
    ...
    jQuery('input[id="secondmailormobile"]').each(function (index) {
        ...
        sRegMail.addClass();
        sRegMail.wrap('<div class="digcon"></div>').before('<div class="dig_fs_countrycodecontainer dig_fs_registersecondcountrycodecontainer"><input type="text" name="digsfcountrycode2" class="input-text countrycode dig_fs_registersecondcountrycode" value="' + dig_mdet.uccode + '" maxlength="6" size="3" placeholder="' + dig_mdet.uccode + '"/></div>');


        if (sRegMail.attr('placeholder')) sRegMail.attr('placeholder', dig_mdet.emailormobile);
        sRegMail.on("keyup change focusin", function () {

            var dclcc = jQuery(this).parent().find('.dig_fs_registersecondcountrycodecontainer');
    ...
    jQuery('.dig_fs_registersecondcountrycode').on("keyup change focusin", function (e) {
        var dfscr = jQuery(this);
        var code = dfscr.val();
        var size = code.length;
        var curRegMail = dfscr.parent().parent().find('input#secondmailormobile');
        size++;
        if (size < 2) size = 2;
        dfscr.attr('size', size);

        if (code.trim().length == 0) {
            dfscr.val("+");
        }
        curRegMail.css({"padding-left": mobilePaddingLeft(dfscr.outerWidth() + ew / 2 + "px")}, 'fast', function () {
        });
    ...
    if (!user_login.attr('disabled')) {
        user_login.wrap('<div class="digcon"></div>').before('<div class="dig_fs_countrycodecontainer forgotcountrycodecontainer"><input type="text" name="dig_countrycodec" class="input-text countrycode forgotcountrycode" value="' + dig_mdet.uccode + '" maxlength="6" size="3" placeholder="' + dig_mdet.uccode + '"/></div>');
    ...
    function digit_validateLogin(usernameid) {
        ...
        if (isNumeric(usernameid.val())) {
            var dclcc = usernameid.parent().find('.dig_fs_countrycodecontainer').find('input');
    ...
    jQuery('.dig_fs_registercountrycode').on("keyup change focusin", function (e) {
    ...
    function updateMailSecondLabel(reg_email) {
        ...
        var digSecondCountryCode = cPar.find('.dig_fs_registersecondcountrycodecontainer');
        ...
        if ((isNumeric(con) && inftype != 1) || dig_mdet.mail_accept == 2) {
            ...
            secondmailormobile.css({"padding-left": mobilePaddingLeft(digSecondCountryCode.find(".dig_fs_registersecondcountrycode").outerWidth() + ew / 2 + "px")});
        }

        if (dig_mdet.mail_accept != 2 && dig_mdet.mobile_accept != 2) {

            if (con == "" || con.length == 0) {
                cPar.find(".dig_fs_mailsecond").stop().slideUp();
                isSecondMailVisible = false;
                return;
            }

            if (!isSecondMailVisible) {
                cPar.find(".dig_fs_mailsecond").stop().slideDown().show();
    ...
    jQuery(document).on("keyup", ".dig_fs_logincountrycode", function (e) {
    ...
    jQuery(".login .inline").each(function () {
        ...
        form.find('.site_front-LostPassword').prepend(jQuery(this));
    ...
    jQuery(".site_front-ResetPassword input").on('focusin', function () {
        if (forgotDone == 1) return;
        jQuery(".site_front-ResetPassword input[type='submit']").each(function () {
            jQuery(this).removeAttr('disabled').removeClass("disabled");
        });
        forgotDone = 1;
    ...
    if (dig_mdet.pass_accept != 2 && dig_mdet.mobile_accept > 0) {
        register.find('input[id="reg_password"]').each(function () {
            jQuery(this).closest('.site_front-form-row').hide();
        });
    }


    register.find(".site_front-Button, button[name='register']").each(function () {
        if (jQuery(this).attr('name') == 'register') {
            if (!jQuery(this).hasClass("otp_reg_dig_fs")) {
                if (jQuery(".otp_reg_dig_fs").length)
                    jQuery(this).val(dig_mdet.RegisterWithPassword).text(dig_mdet.RegisterWithPassword).addClass("fs_reg_pass_btn");
            }
        }
    });
    ...
    register.find(".site_front-Button, button[name='register']").on('click', function (e) {
        ...
        if (jQuery(this).hasClass("otp_reg_dig_fs")) {
            if (!isNumeric(mail) && !isNumeric(secmail)) {
                shodjigNoticeMessage(dig_mdet.Thisfeaturesonlyworkswithmobilenumber);
                return false;
            }

            curRegForm.find(".fs_reg_pass_btn").hide();
            ...
            if (!curRegForm.find("#reg_billing_otp").is(":visible")) {
                ...
                passfsdo = 1;
                var a = curRegForm.find('#reg_password').closest('.site_front-form-row');
                if (a.css('display') == 'none') {
                    curRegForm.find(".otp_reg_dig_fs").hide();
                    a.show();
                    return false;
                }
            }
    ...
    register.on('submit', function (e) {
        ...
        update_time_button = register.find(".site_front-Button");
                ...
                if (dig_mdet.pass_accept == 2 || tpass.length > 0) {
                    try {
                        ...
                        if (strength != null && strength < 3) {
                            shodjigNoticeMessage(dig_mdet.useStrongPasswordString);
                            return false;
                        }
                    } catch (e) {
        ...
        if (passf.length > 0) {
            var pass = passf.val();
            if (!jQuery(this).hasClass("otp_reg_dig_fs") && passf.is(":visible")) {

                if (pass.length == 0) {
                    shodjigErrorMessage(dig_mdet.invalidpassword);
                    return false;
                }
            }
            if (pass.length == 0 && validateEmail(mail) && validateEmail(secmail) && !isNumeric(mail) && !isNumeric(secmail)) {
                shodjigNoticeMessage(dig_mdet.eitherentermoborusepass);
                return false;
            }
        }

        if (validateEmail(mail) && validateEmail(secmail) && secmail.length > 0) {
            shodjigErrorMessage(dig_mdet.InvalidMobileNumber);
            return false;
        }
        if (isNumeric(mail) && isNumeric(secmail) && secmail.length > 0) {
            shodjigErrorMessage(dig_mdet.InvalidEmail);
            return false;
        }
        ...
        if (regverify == 1) {
            if (isNumeric(mail)) {
                verifyOtp(curRegForm.find(".dig_fs_registercountrycode").val(), mail, nounce.val(), otp.val(), 2);
                return false;
            } else if (isNumeric(secmail)) {
                verifyOtp(curRegForm.find(".dig_fs_registersecondcountrycode").val(), secmail, nounce.val(), otp.val(), 2);
                return false;
            }
            return false;
        }
        ...
        akCallback = 'registerdfsCallBack';

        if (isNumeric(mail)) {
            email_reg_field = secmail;
            verifyMobileNoLogin(curRegForm.find(".dig_fs_registercountrycode").val(), mail, nounce.val(), 2);
            email_reg_field = mail;
            return false;
        } else if (isNumeric(secmail)) {
            verifyMobileNoLogin(curRegForm.find(".dig_fs_registersecondcountrycode").val(), secmail, nounce.val(), 2);
            return false;
        }
    ...
    function registerdfsCallBack(response) {
        shodjigitsLoader(true);
        if (response.status === "PARTIALLY_AUTHENTICATED") {
            registerstatus = 1;
    ...
    function verifyMobileNoLogin(countrycode, mobileNo, csrf, dtype) {
        ...
        if (update_time_button.hasClass('dig_fs_mobileWhatsApp')) {
            useWhatsApp = 1;
        }
            ...
            success: function (res) {
                ...
                if (isJSON(res)) {
                    if (res.success === false) {
                        if (res.data.notice) {
                            shodjigNoticeMessage(res.data.message);
                        } else {
                            shodjigErrorMessage(res.data.message);
                        }
                        return;
                    }
                ...
                if (res == -99) {
                    shodjigErrorMessage(dig_mdet.invalidcountrycode);
                    return;
                }
                if (res == -11) {
                    if (dtype == 1) {
                        ...
                        shodjigErrorMessage(dig_mdet.Mobilenumbernotfound);
                        return;
                    }
                } else if (res == 0) {
                    shodjigErrorMessage(dig_mdet.error);
                    return;
                }
                if ((res == -1 && dtype == 2) || (res == -1 && dtype == 11)) {
                    shodjigErrorMessage(dig_mdet.MobileNumberalreadyinuse);
                    return;
                }
                ...
                if (ak == 1) {
                    ...
                    firebase.auth().signInWithPhoneNumber(phone, appVerifier)
                        ...
                        loader.hide();
                        shodjigErrorMessage(error.message);
                    });
                } else {
                    verifyMobNo_success(res, countrycode, mobileNo, csrf, dtype);
                }
    ...
    function verifyMobNo_success(res, countrycode, mobileNo, csrf, dtype) {
        ...
        if (dtype == 1) {
            ...
            if (billing_page == 1) {
                ...
                updateTime(jQuery(".dig_fsbil_bill_resend").attr({
                    "countrycode": countrycode,
                ...
                updateTime(jQuery(".dig_fs_register_resend").attr({
                    "countrycode": countrycode,
                    "mob": mobileNo, "csrf": csrf, "dtype": dtype
                }));
                ...
                curRegForm.find('.otp_reg_dig_fs').not('.dig_otp_submit_button').hide();
            ...
            if (forgotpassihc == 1) {
                ...
                updateTime(jQuery(".dig_fs_forgot_resend").attr({
                    "countrycode": countrycode,
                    "mob": mobileNo, "csrf": csrf, "dtype": dtype
                }));
            ...
            if (wpuseredit == 1) {
                ...
                updateTime(jQuery(".dig_fs_acc_edit_resend").attr({
                    "countrycode": countrycode,
                    "mob": mobileNo, "csrf": csrf, "dtype": dtype
                }));
    ...
    function hideloginpageitems() {
        ...
        if (fs_checkout.length) {
            fs_checkout.find('input[type="password"]').parent().hide();
            fs_checkout.find(".form-row-first").removeClass("form-row-first");
            fs_checkout.find('#rememberme').closest('label').hide();
            fs_checkout.find('[name="login"]').remove();
        }
    ...
    dig_login_wp_elem.on('submit', function () {
        if (logverify == 1) {
            jQuery(".dig_fs_mobileLogin").first().trigger('click');
            return false;
        }
    ...
    jQuery(document).on('click', '.dig_fs_mobileLogin', function () {
        ...
        if (dig_mdet.captcha_accept == 1) {
            ...
            if (captcha_reg_field != null) {
                ...
                if (captcha_reg_field.length == 0) {
                    shodjigErrorMessage("Please enter a valid captcha!");
                    return false;
                }
            }
        }
        ...
        if (logverify == 1) {
            verifyOtp(countryCode, phoneNumber, nounce.val(), cuForm.find("#dig_fs_log_otp").val(), 1);
            return false;
        }
    ...
    function updateCheckoutDetails(response) {
        ...
        if (response.status === "PARTIALLY_AUTHENTICATED") {
            ...
            jQuery("#dig_fs_bill_code").val(code);
            jQuery("#dig_fs_bill_csrf").val(csrf);
    ...
    function forgotPasswordCallBack(response) {
        ...
        if (response.status === "PARTIALLY_AUTHENTICATED") {
            ...
            user_login.attr('name', 'forgotmail');
            jQuery("#digits_fs_code").val(code);
            jQuery("#digits_fs_csrf").val(csrf);
    ...
    function verify_firebase_otp(countryCode, phoneNumber, csrf, otp, dtype) {
        ...
        if (otp == null || otp.length == 0) {
            loader.hide();
            shodjigErrorMessage(dig_mdet.InvalidOTP);
            return;
        }
        ...
        jQuery.ajax({
            ...
            success: function (res) {
                ...
                if (res == 0) {
                    ...
                    shodjigErrorMessage(dig_mdet.ErrorPleasetryagainlater);
                    return;
                ...
                if (dtype == 2) {
                    ...
                    if (forgotpassihc == 1) {
                        ...
                        update_time_button.val(prv_forg_fs);
                        update_time_button.text(prv_forg_fs);

                    } else {
                        forgotPassChange = 1;
                        user_login.parent().parent().hide();
                        jQuery("#digit_forgot_otp_container").hide();
                        jQuery(".dig_fs_forgot_resend").hide();
                        user_login.attr('name', 'forgotmail');
                        jQuery("form.lost_reset_password .changePassword").show();
                        update_time_button.val(prv_forg_fs);
                        update_time_button.text(prv_forg_fs);
                    ...
                    if (wpuseredit == 1) {
                        ...
                        jQuery("form.site_front-EditAccountForm").submit();
                    }
    ...
    jQuery("form.site_front-EditAccountForm input[type='submit'],form.site_front-EditAccountForm button[type='submit']").on('click', function () {
        ...
        var oldMobile = curForm.find('#dig_fs_cur_phone').val();
        var curMobile = curForm.find('.dig_fs_nw_phone').val();
        var countryCode = curForm.find(".dig_fs_logincountrycode").val();
    ...
    function updateAccountCallback(response) {
        ...
        if (response.status === "PARTIALLY_AUTHENTICATED") {
            ...
            jQuery("form.site_front-EditAccountForm #dig_fs_prof_code").val(code);
            jQuery("form.site_front-EditAccountForm #dig_fs_prof_csrf").val(csrf);

            jQuery("form.site_front-EditAccountForm input[type='submit']").click();
        } else if (response.status === "NOT_AUTHENTICATED") {
            // handle authentication failure
        } else if (response.status === "BAD_PARAMS") {
            //Need to update this
    ...
    jQuery(document).on('keyup', '.digits-edit-phone_field input', function () {
        ...
        var countryCode = form.find(".dig_fs_logincountrycode").val();
    ...
    jQuery("form#your-profile input[type='submit'],.digits_update_mobile_submit").on('click', function () {
        ...
        if (dig_mdet.verify_mobile == 1 || is_digits_update.length) {
            ...
            var countryCode = curRegForm.find(".dig_fs_logincountrycode").val();
        ...
        if (!jQuery(this).hasClass("dig_resendotp_disabled")) {
            ...
            jQuery.ajax({
                ...
                success: function (res) {
                    ...
                    loader.hide();
                    if (res == 0) {
                        shodjigNoticeMessage(dig_mdet.Pleasetryagain);
                    } else if (res == -99) {
                        shodjigErrorMessage(dig_mdet.invalidcountrycode);
                    } else {
                        updateTime(dbbtn);
    ...
    var verfiybilling = 0;
    var billing_page = 0;
    if (jQuery("#dig_fs_check_page").length && dig_mdet.mob_verify_checkout == 1 && dig_mdet.mobile_accept > 0) {
        var billing_otp_field = jQuery("#dig_billing_otp");
        billing_otp_field.closest("p").hide();

        var regForm;
        var undigbill = jQuery(".site_front-checkout");
        ...
        createAccount.append("" +
            "<div class='dig_billing_fs_dv'>" +
            digchbtn +
            '</div><div  class="dig_resendotp dig_fsbil_bill_resend" id="dig_man_resend_otp_btn" dis="1">' + dig_mdet.resendOTP + ' <span>(00:<span>30</span>)</span></div>' +
        ...
        jQuery(".site_front-checkout #dig_billing_otp").on('keyup', function (e) {
        ...
        jQuery("#dig_billing_validate_button").on('click', function () {
            ...
            if (!custom_validation || error) {
                shodjigNoticeMessage(dig_mdet.fillAllDetails);
                return false;
            }


            if (jQuery(".dig_opt_mult_con_tac").find('.dig_input_error').length) {
                shodjigErrorMessage(dig_mdet.accepttac);
                return false;
            }
    ...
    jQuery("#buddypress .signup-form").on('submit', function () {
        ...
        var countrycode = bpForm.find(".dig_fs_logincountrycode").val();
        ...
        if (verifybpotp == 1) {
            ...
            shodjigNoticeMessage(dig_mdet.pleaseentermobormail);
            ...
            shodjigErrorMessage(dig_mdet.Invaliddetails);
        }
        return false;
    });
    ...
    jQuery("#buddypress").find("form#settings-form").on('submit', function () {
        ...
        var ccode = form.find(".dig_fs_logincountrycode").val();
    ...
    if (dig_mdet.login_mobile_accept !== 0) {
        ...
        if (bb_wp_lform.length && dig_mdet.login_otp_accept > 0) {
            ...
            bb_wp_lform.find("[type='submit']").parent().append("<div class='dig_otp_block'><div class='digor'> " + dig_mdet.or + "<br /><br /></div>" +
                "<input type='submit' class='dig_fs_mobileLogin button button-primary button-large' id='wp_bb_log_submit' value='" + dig_mdet.loginwithotp + "' />" +
                "<div class='dig_resendotp dig_fs_login_resend dig_bb_login_resend' id='dig_man_resend_otp_btn' dis='1'>" + dig_mdet.resendOTP + " <span>(00:<span>30</span>)</span></div></div></div>");
        }
    }
    jQuery(".wp_reg").on('submit', function () {
        ...
        var ccode = curRegForm.find(".dig_fs_logincountrycode").val();
    ...
    jQuery(".dig_um_loginviaotp").on('click', function () {
        ...
        var countryCode = um_login.find(".dig_fs_logincountrycode").val();
        if (phoneNumber == "" || countryCode == "") {
            shodjigErrorMessage(dig_mdet.InvalidMobileNumber);
            return false;
        }
        ...
        if (isNumeric(phoneNumber)) {
            ...
            shodjigNoticeMessage(dig_log_obj.Thisfeaturesonlyworkswithmobilenumber);
        } else {
            akCallback = 'loginCallback';
    ...
    function validateCustomFields(form) {
        ...
        if (form.attr('wait')) {
            shodjigNoticeMessage(form.attr('wait'));
            return false;
        }

        if (form.attr('error')) {
            shodjigErrorMessage(form.attr('error'));
            return false;
        }
    ...
    function shodjigitsLoader(hideLoader) {
    ...
    var dokan_if_seller = jQuery(".site_front-form-register .show_if_seller");
    if (dokan_if_seller.length) {
        var fs_form = jQuery(".site_front-form-register");
        if (fs_form.find('#dig_cs_name').length) {
            fs_form.find("#first-name").closest('.form-row').remove();
        }
        if (fs_form.find('#dig_cs_lastname').length) {
            fs_form.find("#last-name").closest('.form-row').remove();
        }
        if (dig_mdet.mobile_accept > 0) {
            fs_form.find("#shop-phone").closest('.form-row').remove();
        }
    }
});
